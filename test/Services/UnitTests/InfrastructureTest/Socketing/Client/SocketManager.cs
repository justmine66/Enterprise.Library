...
namespace InfrastructureTest.Socketing.Client
{
    class SocketManager
    {
        ...
        //定义接收数据的对象  
        List<byte> m_buffer;
        //发送与接收的StatedSocketAsyncEventArgs变量定义.  
        private List<StatedSocketAsyncEventArgs> listArgs = new List<StatedSocketAsyncEventArgs>();
        private StatedSocketAsyncEventArgs receiveEventArgs = new StatedSocketAsyncEventArgs();
        int tagCount = 0;
        ...
        /// <summary>  
        /// 初始化发送参数StatedSocketAsyncEventArgs
        /// </summary>  
        /// <returns></returns>  
        StatedSocketAsyncEventArgs initSendArgs()
        {
            var sendArg = new StatedSocketAsyncEventArgs();
        ...
        {
            StatedSocketAsyncEventArgs mys = (StatedSocketAsyncEventArgs)e;
        ...
        private void ProcessError(SocketAsyncEventArgs e)
        {
            ...
            foreach (StatedSocketAsyncEventArgs arg in listArgs)
        ...
        internal void Send(byte[] sendBuffer)
        {
            if (connected)
            {
                ...
                StatedSocketAsyncEventArgs sendArgs = listArgs.Find(a => a.IsUsing == false);
                if (sendArgs == null)
                {
