    ...
    public class TcpConnection : ITcpConnection
    {
        ...
        private void TryParsingReceivedData(ConcurrentQueue<ReceivedData> receiveQueue)
        {
            ...
            try
            {
                ...
                {
                    dataList.Add(data);
                    segmentList.Add(new ArraySegment<byte>(data.Buffer.Array, data.Buffer.Offset, data.DataLength));
                }
                ...
                {
                    _receiveDataBufferPool.Return(dataList[i].Buffer.Array);
                }
            }
            finally
            {
        ...
        struct ReceivedData
        {
            ...
            public ReceivedData(ArraySegment<byte> buffer, int dataLength)
            {
                this.Buffer = buffer;
